...
protected:
    ...
    scalar mass_inv;
    tensor moi_inv;
// give the physical perporties information of damper
    scalar damp_U = 2.8;
    scalar M = 3.0; // density ratio: M = Pi*m / 4;
    scalar K = (2*M_PI/damp_U) * (2*M_PI/damp_U);   // K = (2*Pi*FN)^2*Pi*m/4;
public:
    // getters
    ...
    //    velocity += force*mass_inv*dt;        // velocity updated to t + dt
        velocity = (2.0*force - K*center)*dt/M + velocity_old;
